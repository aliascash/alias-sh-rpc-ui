...
function makeOutputTransactions {
...
function errorHandling {
 if [ -z "$2" ]; then 
 dialog --backtitle "$TITLE_BACK" --colors --title "ERROR" \
 --ok-label 'Continue' --no-shadow --msgbox "$1" 0 0
 else
 echo "$1"
 exit "$2"
 fi
...
function sry {
 dialog --backtitle "$TITLE_BACK" --no-shadow --colors --title "SRY" \
 --msgbox  "\nUnder construction...\n\nSry right now this is a placeholder." 0 0
 refreshMainMenu_GUI
}
function goodbye {
 dialog --backtitle "$TITLE_BACK" --no-shadow --colors --title "GOODBYE" \
 --ok-label 'Leave' --msgbox  "\nHope you enjoyed.\n" 0 0
 reset  
 exit 0
}
function warning {
 ...
 dialog --backtitle "$TITLE_BACK" --colors --title "WARNING" \
 --ok-label 'YES - I´ve understood' --no-shadow --msgbox "$WARNING" 0 0
}
function walletLockedFeedback {
...
function viewAllTransactionsHelper {
 if [ "$1" = "true" ]; then
 echo 'Hide Stakes'
 else
 echo 'Show Stakes'
...
function viewAllTransactions {
 ...
 dialog --no-shadow \
 --begin 0 0 --no-lines --infobox "" "$(tput lines)" "$(tput cols)" \
 \
 --and-widget --colors --extra-button --help-button \
 --ok-label 'Previous' --extra-label 'Next' --help-label 'Main Menu' \
...
function passwordDialog {
 exec 3>&1
 local wallet_password=$(dialog --backtitle "$TITLE_BACK" --no-shadow \
 --insecure --passwordbox "Enter wallet password" 0 0  \
 2>&1 1>&3)
 exit_status=$?
 exec 3>&-
 case $exit_status in
...
function commandInput {
 ...
 exec 3>&1
 buffer=$(dialog --backtitle "$TITLE_BACK" --ok-label "Execute" \
 --cancel-label "Main Menu" \
	          --extra-button --extra-label "Help" --no-shadow \
 --title "Enter Command" --form "$s" 0 0 0 \
 ...
 if [ "$itemBuffer" != "true" ] && [ "$itemBuffer" != "false" ] && \
 [[ ! $itemBuffer =~ ^[0-9]+$ ]]; then
 if [[ "$itemBuffer" != '"'* ]]; then
 USER_DAEMON_PARAMS+='"'
 fi
...
function curlFeedbackHandling {
 dialog --backtitle "$TITLE_BACK" --colors --title "CURL result" \
 --ok-label 'Continue' --no-shadow --msgbox "$1" 0 0
}
function mainMenu_helper {
...
function refreshMainMenu_GUI {
 ...
 --begin 0 0 --no-lines --infobox "" "$(tput lines)" "$(tput cols)" \
 \
 --and-widget --colors --begin "$POSY_TRANSACTIONS" "$POSX_TRANSACTIONS" \
 --title "$TITLE_TRANSACTIONS" --no-collapse\
 --infobox "$(makeOutputTransactions $TEXTWIDTH_TRANS $TEXTHIGHT_TRANS)" \
 "$SIZEY_TRANSACTIONS" "$SIZEX_TRANSACTIONS" \
 \
 --and-widget --colors --begin "$POSY_INFO" "$POSX_INFO" --title "$TITLE_INFO" \
 --no-shadow --no-collapse \
 --infobox "$(makeOutputInfo $TEXTWIDTH_INFO $TEXTHIGHT_INFO)" \
 "$SIZEY_INFO" "$SIZEX_INFO" \
 \
 --and-widget --colors --begin "$POSY_MENU" "$POSX_MENU" --title "$TITLE_MENU" \
 --nocancel --ok-label "Enter" --no-shadow \
...
function unlockWalletForStaking {
 passwordDialog "999999999" "true"
 refreshMainMenu_DATA
}
function refreshMainMenu_DATA {
